...
- [ ] **(Flutter · P1)** Handler notification foreground — affichage in-app banner (flutter\_local\_notifications)
- [ ] **(Flutter · P1)** Handler notification background — notification système OS standard
- [ ] **(Flutter · P1)** Handler notification terminated — deep link vers écran concerné (go\_router)
...
- [ ] **(Flutter · P2)** Scheduling local — notifs locales pour rappels hors-ligne (flutter\_local\_notifications)
...
- [x] **(Flutter · P0)** Navigation (go\_router) — NavigationBar M3 : 🏠 Accueil · 🐇 Lapins · 🤰 Portées · 🧠 IA · ➕ Plus (Aliments/Alertes/Réglages)
...
- [x] UX Portées — bouton retour unique sur “Détails portée” (suppression du doublon qui renvoyait à “Mes Portées”)
- **Dashboard**
...
- [x] **(Flutter · P1)** Alertes prioritaires — badges rouge/orange/vert + actions (marquer lue / action faite)
- [x] **(Flutter · P1)** Timeline 7 jours — portées + événements planifiés (pesée/vaccin)
- [x] **(Flutter · P1)** Widget saisie rapide — pesée en 2 taps accessible depuis le dashboard sans navigation
- [x] **(Flutter · P1)** Score de rentabilité (`POST /functions/v1/rentability-score`) — note /100 + 3 actions prioritaires
- **Onboarding**
...
- **Maintenance**
- [x] Mise à jour Flutter SDK + dépendances (go\_router, Riverpod, Drift, etc.)
- **Offline & thème**
